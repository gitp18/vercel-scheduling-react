...
function App() {
  ...
  const mainFunc = (selectedData) => {
    ...
    let m = recentProductOnMachine; let p = productDimensions;
    const x = m.map((mData, row) => {

        /* Get the list of Product options for this machine */
        const y = p.map((pData, column) => {

          let changeOver = mData.changeOver;

          /* Check what is that recent most product was loaded before, on this same Machine */
          if(mData.recentProductId === '') { // No product was there => "No changeOver"
          ...
          /* You're having the "Product Id" of the recent most product that was loaded before, on this machine */
            ...
            changeOver = (mData.recentProductId === pData.name) ? changeOver : changeOver + 1;
          }
          pData = pData[0]; //mData = mData[0];
          const eligible = checkLoadingEligibility(pData);
          if(eligible){
            loadThisProduct(pData.name, changeOver, row, column, mData, pData);
          }
        });
  ...
  const checkLoadingEligibility = (pData) => {
      ...
      return (pData.produced < pData.demand && pData.shiftItsRemained > 0) ? true : false;
  }
  
  const loadThisProduct = (productId, changeOver, row=2, col=2, mData, pData) => {
    //alert(111);
    console.log(mData, '00000')
    /* 1. Get the result array and Update the cell with the productId received */
    const x = resultArr.map((rowData, i) => {
      let outerFlag = 22222;
      let y = resultArr[i].map((colData, j) => {
          let innerFlag = 100;
          if(i === 2 && j === 2) {
            console.log(i, '--', row, '---', j, '---', col, '---', resultArr[i][j]);
            if(resultArr[1][2] === '' ) {
              resultArr[1][2] = productId;
              updateMachineData(mData[0], productId);
              //updateProductData(pData, productId, mData.ProductionPershift);
              //console.log('innerIf', '--', innerFlag);
              return innerFlag = productId;
            } else {
              console.log('innerElse');
              return innerFlag = 100;
            }
          }else {
            console.log('else');
            return innerFlag = 100;
  ...
  const updateMachineData = (mData, productId) => {
    ...
    if(index != 1){
      ...
      arr.changeOver = parseInt(arr.changeOver) + 1;
